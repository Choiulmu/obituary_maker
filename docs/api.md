...
| `mournerName` | 상주 이름 | O | 10 | 홍철수 |
| `mournerPhone` | 상주 연락처 | O | 13 | 010-1234-5678 |
| `address` | 장례식장 주소 | X | 100 | 서울시 서초구 원지동 산4-1 |
| `account` | 조의금 계좌 | X | 50 | 국민 123456-01-123456 홍철수 |

최대 길이는 글자 수 기준이다. 한글 한 글자를 1로 센다. **공백도 한 글자로 센다.**
값은 적은 그대로 저장한다. 앞뒤 공백을 잘라내지 않으므로 `  홍길동  `은 7자다.

한국어 입력을 기준으로 잡은 값이다. 빈소는 `지하1층 특2호실`처럼 층과 호실을 같이 적는 경우까지 들어가게 둔다.
...
연락처는 형식 검사(`0##-###?#-####`)가 길이까지 정하고, 번호 한 개만 통과한다.

### 상주는 한 명만 받는다

`mournerName`은 한글·영문 글자만 받는다. 공백·쉼표·가운뎃점이 들어가면 `400`(폼에서는 에러 메시지)이다.

```
홍철수          O
홍철수 홍영희    X   상주는 한 분만, 띄어쓰기 없이 적어 주세요.
홍철수, 홍영희   X
홍철수 외 2명    X
```

여러 명을 받으면 연락처는 하나인데 이름은 여럿인 상태가 되고, 조문객은 누구에게 연락해야 하는지 알 수 없다.
대표 상주 한 명과 그 사람의 번호만 받는다. 형제자매를 모두 적어야 하는 요구가 실제로 나오면 그때 상주 목록을 따로 만든다.
...
- Response (성공): `302 Location: /obituaries/a1b2c3d4-e5f6-4718-9abc-0123456789ab/complete`
...
- Input: path `id` = `a1b2c3d4-e5f6-4718-9abc-0123456789ab`
...
- Input: path `id` = `a1b2c3d4-e5f6-4718-9abc-0123456789ab`, header `X-Admin-Token`, form body (고칠 필드만)
...
| 항목 | 전 | 후 | 이유 |
|---|---|---|---|
| 입력 길이 제한 | 없음 | 필드별 10~100자, 공백 포함해서 센다 | 긴 값이 들어오면 모바일 부고장 화면에서 줄이 밀린다 |
| 상주 | 규정 없음 | 이름·번호 모두 한 명만. 공백과 쉼표를 막는다 | 이름은 여럿인데 번호는 하나면 조문객이 누구에게 연락할지 모른다 |
| 부고장 ID | 8자 (`a1b2c3d4`) | 랜덤 UUID (`a1b2c3d4-e5f6-4718-9abc-0123456789ab`) | 발행된 URL은 인증 없이 열리고 상주 연락처가 들어 있다. 손으로 칠 일이 없어 길어도 불편하지 않다 |
